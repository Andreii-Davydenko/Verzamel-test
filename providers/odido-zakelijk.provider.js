...
class OdidoZakelijkProvider extends Provider {
  /**
   * @private
   * @type {string}
   */
   ...
   * @type {string}
   */
  invoiceUrl = 'https://www.odido.nl/zakelijk/my/facturen';

  /**
 * @private
 * @type {string}
 */
  downloadBaseUrl = 'https://www.odido.nl/zakelijk/my/facturen?action=Download&id=';
  ...
  async handle2FA(code) {
    try {
      ...
      await this.page.waitForURL(this.invoiceUrl, { timeout: 50000 });
  ...
  /**
   * @private
   * @return {Promise<Awaited<{date: String, downloadId: String}>[]>}
   */
  async getInvoiceList() {
    try {
      await this.page.waitForLoadState('load');
      const invoiceElementsContainer = this.page.locator('#Section2_Row2_Column1_Cell1_Invoices_Container ul.list-group-inset');

      let invoiceList = [];
      if (await invoiceElementsContainer.isVisible()) {
        invoiceList = await this.page.evaluate(() => {
          const invoiceElements = Array.from(document.querySelectorAll('#Section2_Row2_Column1_Cell1_Invoices_Container ul.list-group-inset > li:not(:last-child)'));
          return invoiceElements.map((element) => {
            const monthYear = element.querySelector('.text-weight-bold')?.textContent.trim();
            const downloadId = element.querySelector('a[href*="/zakelijk/my/facturen?action=Download"]:last-child')?.getAttribute('href').split('id=')[1];
      ...
      const invoiceListFiltered = invoiceList
        .filter((invoice) => invoice.date && invoice.downloadId);

      this.onSuccess('Collect invoice list complete', invoiceListFiltered);
      return invoiceListFiltered;
    } catch (err) {
      await this.onPageError(err, this.page);
      throw new Error('failedToFetchInvoicesFromWebsite');
   ...
   * @return {{date: Date, description: String, href: String}[]}
   */
  normalizeInvoiceList(invoiceList) {
    return invoiceList.map((invoice) => ({
      date: this.dateParser(invoice.date),
      description: invoice.downloadId,
...
module.exports = OdidoZakelijkProvider;
